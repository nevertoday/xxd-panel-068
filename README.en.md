...
That frustration is why I made **Panel 068**. It does not press a template onto your image. It first works out what must not be lost, then translates it into **Re-view the photo with the blank space, ink lines and light colors of oriental freehand brushwork**.
...
The Simplified Chinese file preserves the user's wording verbatim and remains the sole runtime creative and aesthetic authority. The other four files are faithful reading translations for international readers and sharing; they never rewrite the image-generation prompt.

**Keywords:** Oriental freehand brushwork · Using white as black · Ink line and light color · Typesetting of title and postscript · Modern editing order
...
It preserves the identity, posture, and relationships most worth remembering, then recomposes them through Chinese xieyi economy and modern editorial order. Ink line, pale colour, and meaningful emptiness keep the subject recognizable while leaving room for observation, imagination, and resonance.

```text
lock identity, posture, and the primary narrative → deliberately remove background and secondary detail → recompose through positional design and meaningful emptiness → reveal form with pressure-sensitive ink line and restrained pale colour → translate two to four elegant source colours → place inscription-like text along the visual axis
```

## What makes the finished style recognizable

- One clear narrative governs the image; every other element stops at suggestion instead of reconstructing the full photograph.
- Positional design, treating white as active form, solid-void interplay, and density determine both subject and emptiness.
- Fine ink line varies through pressure, dryness, continuity, and hesitation, with some edges naturally disappearing.
- Two to four source colours are lightened, clarified, and given an Eastern restraint through local washes and touches.
- Small inscription-like copy follows the subject, edge of emptiness, or visual axis, with a little modern fine type creating a past-present hierarchy.

Aesthetic constraints and rejection rules live only in the [original source brief](references/original-prompt/zh-CN.md); the Skill and runtime adapter handle delivery variables only. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-068-prompt.en.md)
...
/xxd-panel-068 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB
...
/xxd-panel-068 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP
...
/xxd-panel-068 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-068 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-068 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 068 to a whole directory; shared settings resolve once
/xxd-panel-068 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB
