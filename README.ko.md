...
사진으로 동양적인 운치를 표현하려 할 때 정형화된 산수, 낙관, 서예, 고지 템플릿에 빠지기 쉽습니다. 문화적 기호는 늘어나지만 원본 사진의 대상과 위치, 감정은 오히려 사라집니다. 진정한 사의적 표현은 고전 요소를 쌓는 것이 아니라 어디를 그리고 어디를 비워야 하는지 아는 데 있습니다.

**Panel 068**은 원본 속 대상의 정체성과 자세, 관계에서 출발해 위치 구성과 여백을 형상으로 쓰는 방식, 강약 있는 먹선과 절제된 담채로 화면을 다시 조직합니다. 전통적인 보는 방식과 현대 편집 질서를 함께 두고, 여백이 빈 배경에 그치지 않고 공기와 거리, 여운을 맡게 합니다.

### 이런 상황에 적합합니다

- **인물, 산수, 건축 또는 기물에 동양적인 시정이 필요할 때:** 대상의 정체성과 관계는 여전히 사진에서 가져오며 범용 동양화 소재로 바꾸지 않습니다.
- **문화적 품격은 원하지만 기호를 쌓고 싶지 않을 때:** 위치 구성과 허실, 소밀, 여백을 낙관이나 고전 장식보다 중요하게 다룹니다.
- **전통적인 필의를 현대적인 전달 방식에 맞추고 싶을 때:** 강약 있는 먹선과 2~4가지 담채, 그림 끝에 덧붙이는 글 형식의 작은 글씨를 현대 편집 질서로 통솔합니다.
- **유연한 결과물이 필요할 때:** 상하·좌우 비교, 디자인 단독 출력, 여러 비율, 네 가지 기기용 배경화면을 지원하고 이미지 폴더도 일괄 처리할 수 있습니다.

### 간단한 시작 방법

처음부터 모든 옵션을 읽을 필요는 없습니다. Agent에 이미지를 보내고 다음과 같이 요청하세요.

> XXD Panel 068으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
중국어 간체 파일은 사용자가 제공한 원문을 그대로 보존하며 런타임의 유일한 창작·미적 기준입니다. 다른 네 언어는 국제 독자를 위한 읽기 번역이며 이미지 생성 프롬프트를 다시 쓰지 않습니다.

**키워드:** 동양적인 자유형 붓놀림 · 흰색을 검정색으로 사용 · 먹선과 밝은 색상 · 제목과 추신의 조판 · 현대적인 편집 순서
...
가장 기억할 가치가 있는 대상의 정체성, 자세와 관계를 보존한 뒤, 중국 사의(寫意)의 ‘적은 붓질로 더 많이 말하는’ 방식과 현대적인 편집 질서로 재구성합니다. 먹선과 담채, 의미 있는 여백은 대상을 한눈에 알아보게 하면서도 관찰과 상상, 여운을 위한 공간을 남깁니다.

```text
대상의 정체성·자세·주요 서사 고정 → 배경과 부차적 세부를 의도적으로 덜어내기 → 화면의 위치 경영과 의미 있는 여백으로 재구성 → 필압 변화가 있는 먹선과 절제된 담채로 형태 드러내기 → 원본 사진의 2~4가지 색을 맑고 우아하게 번역 → 시각축을 따라 제발(題跋) 같은 글 배치
```

## 완성 스타일을 알아보는 특징

- 하나의 명확한 서술이 이미지를 지배합니다. 다른 모든 요소는 전체 사진을 재구성하는 대신 제안에 그칩니다.
- 화면의 위치 경영, 여백을 형태로 쓰는 방식, 허실의 호응과 소밀 변화가 대상과 빈 공간을 함께 결정합니다.
- 미세한 먹선은 압력, 건조함, 연속성, 망설임에 따라 달라지며 일부 가장자리는 자연스럽게 사라집니다.
- 원본 사진에서 고른 2~4가지 색을 옅고 맑게 다듬어 부분적인 번짐과 점채로 동양적인 절제를 만듭니다.
- 작은 비문 같은 카피는 주제, 공허의 가장자리, 시각적 축을 따라가며, 약간 모던한 세밀한 활자가 과거-현재의 위계를 만들어낸다.

미적 제약과 금지 항목은 [중국어 원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 런타임 어댑터는 이번 전달 변수만 처리합니다. [Skill 워크플로](SKILL.md) · [영문 런타임 어댑터](references/xxd-panel-068-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 068의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-068 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-068 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-068 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-068 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-068 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-068 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 068 적용, 공통 설정은 한 번만 확정
/xxd-panel-068 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
