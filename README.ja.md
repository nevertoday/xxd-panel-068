...
写真で東洋的な気韻を表そうとすると、定型的な山水、印章、書、古紙のテンプレートに陥りがちです。文化的な記号が増える一方で、元写真の主体、位置、感情はかえって消えてしまいます。本当の写意は古典的要素を積み重ねることではなく、どこを描き、どこを残すべきかを知ることです。

**Panel 068** は原画像にある主体の同一性、姿勢、関係から出発し、位置の構成、余白を形として扱う発想、強弱のある墨線、抑制した淡彩で画面を組み直します。伝統的な見方と現代のエディトリアル秩序を共存させ、余白には単なる空の背景ではなく、空気、距離、余韻を担わせます。

### このような場面に適しています

- **人物、山水、建築、器物に東洋的な詩情が必要：** 主体の同一性と関係は写真に由来し、一般的な水墨画の題材へ置き換えません。
- **文化的な趣は欲しいが、記号は積み上げたくない：** 位置の構成、虚実、疎密、余白を、印章や古典装飾より重視します。
- **伝統的な筆意を現代の発信に生かしたい：** 強弱のある墨線、2〜4 色の淡彩、題跋を思わせる小さな文字を、現代のエディトリアル秩序でまとめます。
- **柔軟な形式で仕上げたい：** 上下・左右比較、デザインのみ、複数比率、4 デバイス向け壁紙に対応し、画像フォルダの一括処理もできます。

### 簡単な始め方

最初からすべての設定を読む必要はありません。画像を Agent に渡し、次のように伝えてください。

> XXD Panel 068 でこの画像を処理して、まず最適な構図とサイズを提案して。

上の作例で視覚方向を確認できます。準備ができたら[使い始める](#使い始める)へ進んでください。全モード、サイズ、引数は後の折りたたみ資料から必要に応じて確認できます。
<!-- xxd-human-intro:end -->
...
簡体字中国語ファイルはユーザー提供の原文を逐語的に保存し、実行時の唯一の創作・美的基準です。他の4言語は国際的な読者向けの読解訳であり、画像生成プロンプトを書き換えません。

**キーワード:** 東洋の手描きの筆致 · 白を黒として使う · 墨線と淡い色 · タイトルとあとがきの組版 · 現代的な編集順序

Panel 068 は、ビジネスの立場、白と黒の計算、仮想と現実の相互作用、密度の集合と分散を利用しています。伝統的な中国絵画のテンプレートをコピーするのではなく、さまざまなストロークのインク ライン、抑制された明るい色、現代的な編集順序を使用して、主題のアイデンティティと後味を保持しています。

<!-- xxd-panel-benefit:start -->
## すぐに判断：XXD Panel 068 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 東洋のフリーハンドの筆致の空白、インクの線、明るい色を使用して写真を再確認します |
| **ひと目で分かる特徴** | 東洋の手描きの筆致 · 白を黒として使う · 墨線と淡い色 · タイトルとあとがきの組版 · 現代的な編集順序 |
...
最も記憶に残すべき主体の同一性、姿勢、関係を保ち、中国写意の「少ない筆で多くを伝える」考え方と現代的な編集秩序で再構成します。墨線、淡彩、意味のある余白によって、対象をひと目で認識できる状態に保ちながら、観察・想像・余韻のための空間を残します。

```text
主体の同一性・姿勢・主題を定める → 背景と二次的要素を意図的に省く → 経営位置と意味のある余白で再構成する → 筆圧の変化を持つ墨線と抑えた淡彩で形を現す → 原写真から 2～4 色を清雅に翻訳する → 視覚軸に沿って題跋風の文字を置く
```

## 完成作品を見分けるポイント

- 1 つの明確な物語がイメージを支配します。他の要素はすべて、写真全体を再構成するのではなく、提案にとどまります。
- 経営位置、計白当黒、虚実の呼応、疎密の変化が、主体と余白の両方を決定します。
- 細いインクの線は、圧力、乾燥、連続性、ためらいによって変化し、一部のエッジは自然に消えます。
- 原写真から選んだ 2～4 色を淡く澄ませ、部分的なぼかしや点染によって東洋的な節度を与えます。
- 小さな碑文のようなコピーが主題、空虚の端、または視覚軸に沿って配置され、少しモダンな細かい活字が過去と現在の階層を作成します。

美的制約と避けるべき表現は[中国語の原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは今回の納品条件だけを扱います。 [Skill ワークフロー](SKILL.md) · [英語ランタイムアダプター](references/xxd-panel-068-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 068 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-068 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-068 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-068 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-068 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 068 を適用し、共通設定は一度だけ解決
/xxd-panel-068 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-068`. インストール後に Agent セッションを再起動してください。
